...
def proxima_linha_cartao(worksheet, linha_cabecalho=15):
  """Encontra a próxima linha livre logo depois da última compra já
  registrada na tabela 'Registro Detalhado de Compras'. Usar isto em vez
  de append_row() evita que o Google Sheets se confunda com a tabela de
  Resumo dos Cartões que fica acima, na mesma aba."""
  valores = worksheet.get_all_values()
  ultima_com_dados = linha_cabecalho
  for i in range(linha_cabecalho + 1, len(valores) + 1):
    linha = valores[i - 1]
    if any(c.strip() for c in linha[:3]):
      ultima_com_dados = i
...
def obter_cartoes_cadastrados(worksheet):
  ...
  de Crédito' (linhas 2 a 13, coluna A), para que a lista no app sempre
  reflita o que está na planilha — mesmo que a pessoa renomeie ou
  adicione um cartão novo lá."""
  try:
    valores = worksheet.get("A2:A13")
  except Exception:
    return []
  cartoes = []
  for linha in valores:
    if linha and linha[0].strip():
      nome = linha[0].strip()
      if not nome.upper().startswith(("RESUMO", "REGISTRO")):
        cartoes.append(nome)
  return cartoes
    ...
    if st.button("Registrar Compra Parcelada"):
      if cartao and descricao and valor_total > 0:
        ...
        try:
          ...
          proxima_linha = proxima_linha_cartao(cartoes_sheet)
          for i in range(int(parcelas)):
            ...
            linha_destino = proxima_linha + i
            cartoes_sheet.update(
                range_name=f"A{linha_destino}:I{linha_destino}",
                values=[[
                    data_str,
                    cartao,
                    descricao,
