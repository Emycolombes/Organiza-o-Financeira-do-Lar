...
# Tag especial que o iOS/Safari usa para o ícone de "Adicionar à Tela de
# Início" — o favicon normal (page_icon acima) não é suficiente para isso.
if LOGO_BASE64:
  st.markdown(
      f"""
      <link rel="apple-touch-icon" href="data:image/png;base64,{LOGO_BASE64}">
      <link rel="apple-touch-icon-precomposed" href="data:image/png;base64,{LOGO_BASE64}">
      """,
...
def obter_cartoes_cadastrados(worksheet):
  """Lê os nomes dos cartões já cadastrados na tabela 'Resumo dos Cartões
  de Crédito' (linhas 4 a 13, coluna A — pulando o título e o cabeçalho),
  para que a lista no app sempre reflita o que está na planilha — mesmo
  que a pessoa renomeie ou adicione um cartão novo lá."""
  try:
    valores = worksheet.get("A4:A13")
  except Exception:
    return []
  cartoes = []
  for linha in valores:
    if linha and linha[0].strip():
      cartoes.append(linha[0].strip())
  return cartoes


def obter_dia_vencimento(worksheet, cartao):
  """Lê o dia de vencimento do cartão indicado na tabela 'Resumo dos
  Cartões de Crédito' (coluna 'Data Vencimento'). Aceita tanto um número
  simples (ex: 10) quanto uma data completa (ex: 10/09/2026) — em ambos
  os casos usa os primeiros dígitos como o dia. Devolve None se o cartão
  não for encontrado ou o vencimento ainda não estiver preenchido."""
  try:
    valores = worksheet.get("A4:G13")
  except Exception:
    return None
  for linha in valores:
    if linha and linha[0].strip() == cartao:
      if len(linha) >= 7 and linha[6].strip():
        digitos = "".join(c for c in linha[6].strip() if c.isdigit())[:2]
        if digitos:
          try:
            dia = int(digitos)
            if 1 <= dia <= 31:
...
def proxima_linha_cartao(worksheet, linha_inicio=16):
  """Acha a próxima linha livre na tabela 'Registro Detalhado de
  Compras', olhando só a coluna Data (coluna A). Usar só a Data evita
  confusão com as colunas Cartão/Mês Referência, que na planilha podem
  já vir com um valor de exemplo preenchido mesmo em linhas ainda não
  usadas de verdade."""
  valores = worksheet.get_all_values()
  ultima_com_dados = linha_inicio - 1
  for i in range(linha_inicio, len(valores) + 1):
    linha = valores[i - 1]
    if linha and linha[0].strip():
      ultima_com_dados = i
  return ultima_com_dados + 1
...
if sh:
  ...
  if tipo == "Despesa":
    ...
    cartoes_sheet_preview = sh.worksheet("Cartões de Crédito")
    cartoes_cadastrados = obter_cartoes_cadastrados(cartoes_sheet_preview)
    opcoes_cartao = cartoes_cadastrados + ["➕ Outro (cartão novo)"]
    cartao_escolhido = st.selectbox("Cartão", opcoes_cartao)
    if cartao_escolhido == "➕ Outro (cartão novo)":
      cartao = st.text_input(
          "Nome do novo cartão (lembre-se de cadastrá-lo também na tabela"
          " 'Resumo dos Cartões' na planilha)"
      )
    else:
    ...
    if st.button("Registrar Compra Parcelada"):
      if cartao and descricao and valor_total > 0:
        ...
        try:
          ...
          # Decide se a 1ª parcela cai na fatura deste mês ou na seguinte,
          # com base no dia de vencimento cadastrado para o cartão (se
          # houver). Sem vencimento cadastrado, assume que cai no mês
          # seguinte (aproximação mais comum).
          dia_vencimento = obter_dia_vencimento(cartoes_sheet, cartao)
          if dia_vencimento is not None and data_compra.day <= dia_vencimento:
            mes_base = data_compra.month - 1
          else:
            mes_base = data_compra.month

          linha_destino = proxima_linha_cartao(cartoes_sheet)
          for i in range(int(parcelas)):
            mes_idx = (mes_base + i) % 12
            ...
            cartoes_sheet.update(
                range_name=f"A{linha_destino + i}:I{linha_destino + i}",
                values=[[
                    data_str,
                    cartao,
                    descricao,
